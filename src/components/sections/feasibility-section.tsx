...
const ANALYSES = [
  { id: 'ZONING_LAND_USE_ANALYSIS', cat: 'REGULATORY', status: 'VERIFIED', date: '14:22:01' },
  { id: 'PHASE_I_ENVIRONMENTAL_ESA', cat: 'ENVIRON', status: 'VERIFIED', date: '12:05:44' },
  { id: 'GEOTECH_SOIL_REPORT', cat: 'ENGINEERING', status: 'VERIFIED', date: '09:15:22' },
  { id: 'TITLE_SURVEY_REVIEW', cat: 'LEGAL', status: 'VERIFIED', date: 'Yesterday' },
  { id: 'MARKET_FEASIBILITY_STUDY', cat: 'FINANCIAL', status: 'VERIFIED', date: 'Yesterday' },
  { id: 'FINANCIAL_PRO_FORMA_V3', cat: 'FINANCIAL', status: 'VERIFIED', date: '2 Days Ago' },
  { id: 'TRAFFIC_IMPACT_STUDY', cat: 'ENGINEERING', status: 'AUDIT', date: 'Active' },
  { id: 'CEQA_INITIAL_STUDY', cat: 'ENVIRON', status: 'PENDING', date: '—' },
] as const;
        ...
        <Card className="bg-[#12121A] border-[#1E1E2E] relative overflow-hidden flex-1 min-h-0 md:min-h-[250px]">
          <CardHeader className="border-b border-[#1E1E2E]/50 bg-[#0A0A0F]/50 py-3 relative z-20 shrink-0">
            <div className="flex items-center justify-between">
              <CardTitle className="text-[10px] tracking-[0.3em]">Geospatial_Feasibility_Feed</CardTitle>
          ...
          <CardContent className="p-0 relative h-full overflow-hidden">
            <div className="absolute inset-0">
              ...
              <svg viewBox="0 0 800 400" className="absolute inset-0 w-full h-full" preserveAspectRatio="xMidYMid slice">
                ...
                {/* ─── Zoning & Entitlement Card (terminal-style) ─── */}
                <line x1={340} y1={190} x2={340} y2={198} stroke="rgba(0,124,90,0.3)" strokeWidth="0.75" />
                <rect x={268} y={198} width={210} height={88} rx={2} fill="rgba(0,0,0,0.8)" stroke="rgba(0,124,90,0.4)" strokeWidth="0.5" />
                <line x1={268} y1={198} x2={273} y2={198} stroke="rgba(0,124,90,0.8)" strokeWidth="1.5" />
                <text x={278} y={212} fill="rgba(0,124,90,0.95)" fontSize="5.5" fontFamily="monospace">$ zone --parcel 0412-031 --feasibility</text>
                <text x={278} y={223} fill="rgba(0,124,90,0.7)" fontSize="5.5" fontFamily="monospace">ZONE: C2-2D-O | FAR: 6:1 | HT_MAX: 395ft</text>
                <text x={278} y={234} fill="rgba(0,124,90,0.7)" fontSize="5.5" fontFamily="monospace">BUILDABLE: 96,000sf | STORIES: 30 max</text>
                <text x={278} y={245} fill="rgba(0,124,90,0.7)" fontSize="5.5" fontFamily="monospace">SETBACK: F=10ft S=6ft R=15ft | LOT_COV: 75%</text>
                <text x={278} y={256} fill="rgba(0,124,90,0.55)" fontSize="5.5" fontFamily="monospace">USE: MIXED(B/R-2) | SEISMIC: D | FIRE: TYPE_IA</text>
                <text x={278} y={267} fill="rgba(0,124,90,0.55)" fontSize="5.5" fontFamily="monospace">PARKING: 1:500sf = 192 stalls required</text>
                <text x={278} y={278} fill="#007C5A" fontSize="5.5" fontFamily="monospace" fontWeight="bold">STATUS: FEASIBLE ✓</text>
                ...
                {/* Parking constraint card (terminal-style) */}
                <line x1={555} y1={325} x2={555} y2={332} stroke="rgba(245,158,11,0.25)" strokeWidth="0.75" />
                <rect x={488} y={332} width={175} height={50} rx={2} fill="rgba(0,0,0,0.8)" stroke="rgba(245,158,11,0.35)" strokeWidth="0.5" />
                <line x1={488} y1={332} x2={493} y2={332} stroke="rgba(245,158,11,0.8)" strokeWidth="1.5" />
                <text x={498} y={346} fill="rgba(245,158,11,0.85)" fontSize="5.5" fontFamily="monospace">$ zone --parcel 0412-045 --flags</text>
                <text x={498} y={357} fill="rgba(245,158,11,0.6)" fontSize="5.5" fontFamily="monospace">PARKING_RATIO: 192 req / 169 prov = -12%</text>
                <text x={498} y={368} fill="rgba(245,158,11,0.6)" fontSize="5.5" fontFamily="monospace">VARIANCE_REQ: SIDE=0ft | DEFICIT: 23 stalls</text>
                <text x={498} y={379} fill="#f59e0b" fontSize="5.5" fontFamily="monospace" fontWeight="bold">{"STATUS: AT_RISK \u26A0"}</text>
          ...
          <CardHeader className="border-b border-[#1E1E2E]/50 bg-[#0A0A0F]/50 py-3 shrink-0">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3 text-[8px] font-mono hidden sm:flex">
                <span className="text-primary uppercase">6 Verified</span>
                <span className="text-white/20">|</span>
                <span className="text-yellow-500/80 uppercase">1 Audit</span>
                <span className="text-white/20">|</span>
                <span className="text-white/40 uppercase">1 Pending</span>
              </div>
            </div>
          </CardHeader>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <div className={cn(
                          ...
                          doc.status === 'VERIFIED' ? 'bg-primary' :
                          doc.status === 'AUDIT' ? 'bg-yellow-500' :
                          'bg-muted-foreground'
                        )} />
                        <span className={cn(
                          "text-[10px] md:text-[8px] font-bold uppercase tracking-widest",
                          doc.status === 'VERIFIED' ? 'text-primary/80' :
                          doc.status === 'AUDIT' ? 'text-yellow-500/80' :
                          'text-white/40'
                        )}>{doc.status}</span>
                      </div>
                    </TableCell>
